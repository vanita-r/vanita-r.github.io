document.addEventListener('DOMContentLoaded', () => {
    ...
    htmlButton.addEventListener('click', (event) => {
        ...
        departmentInputs.forEach((deptInput) => {
            ...
            // Loop to go through each course
            if (dept && number && name && reason) {
                // Add indentation (2 spaces) for the <li> and (4 spaces) for the <strong>
                // \n means "new line"
                coursesHtmlString += `\n            <li>
        ...
        // Join the links with the divider
        const linksHtmlString = linkElements.join(` &nbsp;${divider.trim()}&nbsp; `);
...
<p>${linksHtmlString}</p>
        ...
        resultsContainer.style.display = 'block'; // Make sure results are visible
